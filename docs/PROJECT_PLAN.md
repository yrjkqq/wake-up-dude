...
* [ ] 引入 `expo-sqlite` 记录历史生成的"毒舌语录"，支持回听"名场面"。
* [ ] 实现音频缓存策略：本地保留最近 7 天音频，自动清理过期文件。
* [ ] 实现多种人设模式选择（毒舌监督员 / 军训教官 / 温柔女友 / 社畜互助）。
* [ ] 优化 UI/UX：增加滑动关闭闹钟的交互，适配暗色模式，集成 `expo-haptics` 震动反馈。
* [ ] 错误兜底机制：如果网络请求失败，自动回退到默认的刺耳备用铃声（预置在 bundle 中）。
* [ ] 准备上架所需的各种 Icon、截图和隐私协议。
